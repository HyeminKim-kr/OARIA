...
# ============================================================
# Papers 임베딩 Dispatch 태스크
# ============================================================


@app.task(bind=True, queue="embed")
def dispatch_paper_jobs(self):
    """Papers 임베딩 대기 큐의 작업을 워커에게 dispatch
    ...
    while True:
        job_id = manager.pop_pending_job(JobType.PAPER_EMBED)
        if not job_id:
            break

        state = manager.get_job_state(job_id, JobType.PAPER_EMBED)
        if not state:
            logger.warning("dispatch_paper_job_not_found", job_id=job_id)
            continue

        if state.get("status") in (JobStatus.PROCESSING, JobStatus.COMPLETED):
            logger.debug(
                "dispatch_paper_job_skipped",
                job_id=job_id,
                status=state.get("status"),
            )
            skipped += 1
        ...
        if not manager.acquire_lock(job_id, JobType.PAPER_EMBED, worker_id):
            logger.debug("dispatch_paper_lock_failed", job_id=job_id)
            manager.redis.rpush(manager._queue_key(JobType.PAPER_EMBED), job_id)
            skipped += 1
            continue

        # 실제 작업 태스크 호출
        from .embed import run_paper_embed_v2
        run_paper_embed_v2.delay(job_id)
        dispatched += 1

        logger.info(
            "paper_job_dispatched",
            job_id=job_id,
            job_type=JobType.PAPER_EMBED,
        )

    if dispatched > 0 or skipped > 0:
        logger.info(
            "paper_dispatch_completed",
            dispatched=dispatched,
            skipped=skipped,
        )
...
@app.task(bind=True, queue="embed")
def recover_stuck_paper_jobs(self):
    """Papers 임베딩 Stuck 작업 감지 및 복구
    ...
    stuck_jobs = manager.get_stuck_jobs(JobType.PAPER_EMBED)

    for job_id in stuck_jobs:
        if manager.recover_stuck_job(job_id, JobType.PAPER_EMBED):
            recovered += 1
            logger.info("stuck_paper_job_recovered", job_id=job_id)
        else:
            max_retries_exceeded += 1
            logger.warning("stuck_paper_job_max_retries", job_id=job_id)

            # 배치 작업의 경우 별도 DB 동기화 필요 없음
            # (개별 논문 상태는 워커에서 관리)

    if recovered > 0 or max_retries_exceeded > 0:
        logger.info(
            "paper_recovery_completed",
            recovered=recovered,
            max_retries_exceeded=max_retries_exceeded,
        )
...
@app.task(bind=True, queue="embed")
def sync_papers_from_db(self):
    """Papers 임베딩 배치 작업 동기화 (DB → Redis)

    Celery Beat에서 5분마다 실행.
    Redis 장애 복구 시 사용.

    참고: papers 임베딩은 배치 단위로 관리되므로,
    개별 논문의 embedding_status가 아닌 Redis의 배치 작업 상태를 복원합니다.
    """
    # papers 임베딩은 배치 작업 ID가 Redis에만 있으므로
    # DB에서 복원할 데이터가 없음.
    # 대신, processing 상태로 stuck된 논문들을 pending으로 되돌려야 함.
    ...
    try:
        with pool.connection() as conn:
            with conn.cursor() as cur:
                # processing 상태가 1시간 이상 된 논문은 stuck으로 간주
                cur.execute("""
                    UPDATE papers
                    SET embedding_status = 'pending',
                        embedding_error = 'Reset by sync task (stuck recovery)'
                    WHERE embedding_status = 'processing'
                      AND embedding_at < NOW() - INTERVAL '1 hour'
                    RETURNING id
                """)
        ...
        if reset_count > 0:
            logger.info("papers_sync_reset", reset_count=reset_count)

    except Exception as e:
        logger.error("papers_sync_error", error=str(e))
