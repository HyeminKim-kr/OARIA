...
# Chunk, ChunkingResult는 동적 임포트 (순환 참조 방지)
def _import_chunker():
    import importlib.util
    spec = importlib.util.spec_from_file_location("chunker", OAR_29_PATH / "src" / "chunker.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module

_chunker_module = None

def get_chunker_module():
    global _chunker_module
    if _chunker_module is None:
        _chunker_module = _import_chunker()
    return _chunker_module
...
class WeaviateClient:
    ...
    def insert_chunk(
        self,
        chunk,  # Chunk 객체 (OAR-29 Chunker 결과)
        paper_metadata: dict,
    ...
    def insert_chunking_result(
        self,
        result,  # ChunkingResult 객체 (OAR-29 Chunker 결과)
        paper_metadata: dict,
        batch_size: int = 10,
    ) -> list[str]:
        """청킹 결과 전체 삽입
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            print(f"  📦 삽입: {min(i + batch_size, len(chunks))}/{len(chunks)} 청크")
