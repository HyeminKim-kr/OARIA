"""
Batch Crawler Scheduler (OAR-21 / OAR-102 + ADR-001b)
...
- ADR-001b: Open Access only, full-text extraction
...
class CrawlStats:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "full_text_extracted": self.full_text_extracted,
...
class BatchScheduler:
    ...
    def _build_filter(self, from_date: Optional[date] = None) -> str:
        """Build OpenAlex filter string (ADR-001b: Open Access only)."""
        filters = [
            f"concepts.id:{self.config.concepts[0]}",
            "has_abstract:true",
            "type:article",
            "open_access.is_oa:true",  # ADR-001b
        ]
    ...
    def _print_stats(self):
        ...
        print(f"     Open Access:  {s.open_access:,}")
        print(f"     Trusted Src:  {s.trusted_source:,}")
        print(f"     Parsed:       {s.papers_parsed:,}")
        print(f"     Saved:        {s.papers_saved:,}")
        print(f"     Full Text:    {s.full_text_extracted:,}")
                ...
                for raw in results:
                    ...
                    if oa_url and PDF_SUPPORT:
                        ...
                        if full_text and len(full_text) > 100:
                            ...
                            self.stats.full_text_extracted += 1
