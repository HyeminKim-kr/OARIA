...
OAR-29: 섹션 기반 Recursive Chunking 구현
- offset 추적 (근거 재현용)
...
class TextChunker:
    ...
    def _recursive_split(self, text: str, separator_idx: int) -> list[str]:
        """재귀적으로 텍스트 분할 (레거시, 사용하지 않음)"""
        return self._recursive_split_no_overlap(text, separator_idx)
    ...
    def _apply_overlap(self, chunks: list[str]) -> list[str]:
        ...
        result = [chunks[0]]
        overlap_chars = self.chunk_overlap_tokens * 4  # 대략적인 변환
        ...
        for i in range(1, len(chunks)):
            ...
            # 이미 포함되어 있는지 확인
            if not curr_chunk.startswith(overlap_text[:20] if len(overlap_text) > 20 else overlap_text):
    ...
    def _merge_small_chunks(self, chunks: list[str]) -> list[str]:
        ...
        for chunk in chunks:
            ...
            if self.count_tokens(chunk) < self.min_chunk_tokens:
                # 이전 청크에 병합
