...
export default function HeatmapChart({ data }: Props) {
  ...
  useEffect(() => {
    ...
    const colorScale = d3
      .scaleSequential()
      .domain([0, maxCount])
      .interpolator(d3.interpolateRgb("#EEF2FF", "#7EC8E3"));
    ...
    svg
      ...
      .on("mouseover", function (_event, _d) {
        d3.select(this).attr("stroke", "var(--oaria-teal)").attr("stroke-width", 2);
      ...
      .on("mousemove", function (event) {
        ...
        tip
          .html(`<strong>${d.journal}</strong><br/>${d.year}년: <strong>${d.count}</strong>건`)
          .style("left", `${mx + 16}px`)
          .style("top", `${my - 10}px`);
      })
