...
# ============================================================
# OARIA 개발 환경 시작 및 모니터링 스크립트 (Staging)
# ============================================================
#
# 자세한 도움말: ./scripts/dev_staging.sh help
#
# 기본 명령어 (인터랙티브):
#   ./scripts/dev_staging.sh start     - 개발 환경 시작 (로컬/Docker 선택)
#   ./scripts/dev_staging.sh stop      - 개발 환경 중지
#   ./scripts/dev_staging.sh restart   - 재시작
#   ./scripts/dev_staging.sh status    - 상태 확인
#   ./scripts/dev_staging.sh logs      - 로그 보기
#   ./scripts/dev_staging.sh monitor   - 실시간 모니터링
#
# 로컬 개발 모드 (직접 실행):
#   ./scripts/dev_staging.sh local start/stop/restart       - 로컬 모드 (인프라 Docker + 로컬 앱)
#
# Docker 개발 모드 (직접 실행):
#   ./scripts/dev_staging.sh docker-dev start/stop/restart/rebuild  - 개발 모드 (Hot Reload)
#   ./scripts/dev_staging.sh prod start/stop/restart/rebuild        - 프로덕션 모드
#
# 인프라 제어:
#   ./scripts/dev_staging.sh docker start/stop/restart  - 인프라만 (DB, Redis 등)
#
# 로컬 서비스 개별 제어:
#   ./scripts/dev_staging.sh admin start/stop/restart    - Admin 서비스만 (로컬)
#   ./scripts/dev_staging.sh service start/stop/restart  - User 서비스만 (로컬)
#
# 개별 컨테이너 제어:
#   ./scripts/dev_staging.sh container <name> start/stop/restart/rebuild/logs
#   예: ./scripts/dev_staging.sh container celery-backfill restart
#
# 유틸리티:
#   ./scripts/dev_staging.sh install   - 의존성 설치
#   ./scripts/dev_staging.sh migrate   - DB 마이그레이션
#
# ============================================================
...
acquire_lock() {
    ...
    while [ -f "$LOCK_FILE" ]; do
        ...
        if [ $count -ge $timeout ]; then
            echo -e "${RED}오류: 다른 dev_staging.sh 프로세스가 실행 중입니다 (PID: $lock_pid)${NC}"
...
start_docker() {
    ...
    echo -e "  ${GREEN}✓${NC} Docker Compose 시작 완료"
}
...
start_docker_dev() {
    ...
    echo -e "  ${GREEN}✓${NC} 개발 모드 시작 완료 (볼륨 마운트로 Hot Reload 지원)"
}
...
start_admin_backend() {
    ...
    echo -e "  ${GREEN}✓${NC} Admin Backend 시작됨 (log: .dev-logs/admin-backend.log)"
}
...
start_admin_frontend() {
    ...
    echo -e "  ${GREEN}✓${NC} Admin Frontend 시작됨 (log: .dev-logs/admin-frontend.log)"
}
...
start_service_backend() {
    ...
    echo -e "  ${GREEN}✓${NC} Service Backend 시작됨 (log: .dev-logs/service-backend.log)"
}
...
start_service_frontend() {
    ...
    echo -e "  ${GREEN}✓${NC} Service Frontend 시작됨 (log: .dev-logs/service-frontend.log)"
}
...
show_help() {
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${CYAN}  OARIA 개발 환경 관리 스크립트 (Staging)${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    echo ""
    echo -e "${BLUE}[Docker 컨테이너 개별 제어]${NC}"
    echo ""
    ...
    echo -e "${BLUE}[유틸리티]${NC}"
    echo ""
    echo -e "  ${GREEN}$0 install${NC}      모든 의존성 패키지 설치"
    echo -e "  ${GREEN}$0 migrate${NC}      DB 마이그레이션 실행"
    echo ""
}
